...
def prepare():
    for lang in ['go', 'java', 'python', 'ruby', 'javascript', 'php']:
        for split in ['train', 'valid', 'test']:
            with open(f'processed/{split}.{lang}-en_XX.{lang}', 'w', encoding='utf-8') as src_writer:
                tgt_writer = open(
                    f'processed/{split}.{lang}-en_XX.en_XX',
                    'w',
                    encoding='utf-8',
                )
                filename = f'{lang}/{split}.jsonl'
                with open(filename) as f:
                    for line in tqdm(f, total=count_file_lines(filename), desc=f"{lang}-{split}"):
