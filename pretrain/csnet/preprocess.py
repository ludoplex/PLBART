...
def prepare(args):
    ...
    with open(f'{args.target_pl_dir}/{args.split}.functions.tok', 'w', encoding='utf-8') as pl_writer:
        nl_writer = open(
            f'{args.target_nl_dir}/{args.split}.docstring.tok',
            'a',
            encoding='utf-8',
        )
        for file in glob.glob(f"{src_dir}/{args.lang}_{args.split}_*.jsonl"):
            filename, _ = os.path.splitext(os.path.basename(file))
            with open(file) as f:
                data = [json.loads(line.strip()) for line in f]

            results = []
            with tqdm(total=len(data), desc=f"{args.lang}-{filename}") as pbar:
        ...
        if args.split == 'train':
            num_unimodal_ex = 0
            with open(f"{src_dir}/{args.lang}_dedupe_definitions_v2.pkl", 'rb') as f:
                data = pickle.load(f)

                results = []
                with tqdm(total=len(data), desc="unimodal-data") as pbar:
                    for out in pool.imap(process_unimodal_instance, data, 1000):
                        pbar.update()
                        results.append(out)
